<script>
    ...
    function calculateInterest() {
        interest = Number(deposit.replaceAll(',', '')) * interestRate / 365
...
<div class="m-auto w-full max-w-sm p-5 bg-white border border-gray-200 rounded-lg shadow sm:p-5">
    {#if visible}
        <div transition:fade id="alert-2" class="flex p-4 mb-4 text-red-800 rounded-lg bg-red-50" role="alert">
            ...
            <button on:click={init} type="button"
    ...
    <div class="grid items-end gap-6">
        <div class="relative z-0">
            <input type="text" id="default_standard" bind:value={deposit} on:keyup={changeDeposit}
                   class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                   placeholder=" "/>
            <label for="default_standard"
                   class="absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">예치금액</label>
        </div>
    </div>
